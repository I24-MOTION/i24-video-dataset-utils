...
def track_chunk(data_dir,tree,params,q = None,SHOW = False):
   ...
   tracklets,seams = get_tracklets(data_dir,tree)
   seam_idx = None
   if len(seams) == 2:
       seam_idx = seams[0]
       print("\nUSING SEAM IDX {}\n".format(seam_idx)) 
               ...
               if not FINAL:
                   ...
                   for zone in zones["WB"]["source"].values():
                       if tracklets[i][0,1] > zone[0] and tracklets[i][0,1] < zone[1] and tracklets[i][0,2] > zone[2] and tracklets[i][0,2] < zone[3]:
                           SOURCE = True
                           break
                   if not SOURCE and torch.max(tracklets[i][:,1]) > end_x  - x_margin or  torch.min(tracklets[i][:,0]) < start_time + t_margin: # chunk boundaries are the source
                       SOURCE = True
                   ...
                   if SOURCE:
                       ...
                       for zone in zones["WB"]["sink"].values():
                           if tracklets[i][-1,1] > zone[0] and tracklets[i][-1,1] < zone[1] and tracklets[i][-1,2] > zone[2] and tracklets[i][-1,2] < zone[3]:
                               SINK = True
                               break
                       if not SINK and torch.min(tracklets[i][:,1]) < start_x + x_margin or torch.max(tracklets[i][:,0]) > end_time - t_margin: # chunk boundaries are the sink
                           SINK = True
   ...
   tm.split("cleanup")
   q.put([tree,tracklets,tracklets_complete,iteration,tm.bins()])
   q.put([tree,"DONE"]) 
